...
function renderSortPills() {
  ...
  container.innerHTML = SORT_OPTIONS.map(opt => {
    ...
    const arrow = currentSortOrder === 'asc' ? 'Γåæ' : 'Γåô';
  ...
  dropdown.innerHTML = results.slice(0, 8).map((a, idx) => {
    ...
    <div class="drop-item" data-idx="${idx}" data-id="${a.id}" data-type="${mediaType}">
      ...
      <div class="drop-info">
        ...
        <div class="drop-meta"><span class="drop-type-badge ${typeClass}">${typeLabel}</span>${escHtml(year)} ┬╖ Γÿà ${a.vote_average ? a.vote_average.toFixed(1) : 'N/A'}</div>
      </div>
...
async function markSelectedWatched() {
  ...
  await save();
  toggleSelectMode();
  showToast(`Marked ${count} title(s) as watched Γ£ô`);
  pendingStatsBadge = false;
}
      ...
      <article class="card ${a.watched ? 'watched' : ''} ${a.archived ? 'dropped' : ''} ${deleteMode ? 'delete-mode' : ''} ${selectedForDelete.has(a.id) ? 'selected' : ''}" id="card-${a.id}" onclick="openModal(${a.id}, '${a.media_type}', event)">
        ...
        <div class="card-content">
          ...
          ${showEpCounter && isTV ? `<div class="card-ep-counter" onclick="event.stopPropagation()">
            <button class="ep-btn" style="visibility: ${(a.episodesWatched || 0) <= 0 ? 'hidden' : 'visible'};" onmousedown="startProgress(${a.id},-1,event)" onmouseup="stopProgress(event)" onmouseleave="stopProgress(event)" ontouchstart="startProgress(${a.id},-1,event)" ontouchend="stopProgress(event)">ΓêÆ</button>
...
async function openModal(id, mediaType, event) {
  ...
  content.innerHTML = `<div class="modal-loading"><img src="assets/images/blocks_shuffle_loading.svg" alt="Loading..." class="blocks-loading" />Loading detailsΓÇª</div>`;
    ...
    content.innerHTML = `
      <div class="modal-hero">
        ...
        <div class="modal-hero-info">
          ${detail.vote_average ? `<div class="modal-score">Γÿà ${detail.vote_average.toFixed(1)}</div>` : ''}
      ...
      <div class="modal-body">
          ...
          <div style="grid-column: 1 / -1; display:flex; flex-direction:column; gap:8px;">
            <div style="background: var(--elevated); padding: 12px 16px; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: space-between; border: 1px solid var(--border);">
              ...
              <div class="progress-controls">
                <button class="progress-btn" id="modalSeasonMinus" style="visibility: ${isMinSeason ? 'hidden' : 'visible'};" onclick="changeSeason(${id},-1,event)">ΓêÆ</button>
            ...
            <div style="background: var(--elevated); padding: 12px 16px; border-radius: var(--radius-md); display: flex; align-items: center; justify-content: space-between; border: 1px solid var(--border);">
              ...
              <div class="progress-controls">
                <button class="progress-btn" id="modalEpMinus" style="visibility: ${(existingItem.episodesWatched || 0) <= 0 ? 'hidden' : 'visible'};" onmousedown="startProgress(${id},-1,event)" onmouseup="stopProgress(event)" onmouseleave="stopProgress(event)" ontouchstart="startProgress(${id},-1,event)" ontouchend="stopProgress(event)">ΓêÆ</button>
        ...
        ${syn.length >= 180 ? '<button class="read-more" onclick="toggleSynopsis()">Read more Γåô</button>' : ''}

        ${collectionOrder.length > 1 ? `
        <div class="watch-order">
          <div class="watch-order-title">≡ƒô╜∩╕Å Collection Watch Order</div>
...
function toggleSynopsis() {
  ...
  btn.textContent = box.classList.contains('expanded') ? 'Read less Γåæ' : 'Read more Γåô';
}
...
async function markWatchedFromModal(id, mediaType) {
  ...
  if (item) {
    ...
    showToast('Marked as watched Γ£ô');
    pendingStatsBadge = false;
  }
}
      ...
      container.innerHTML = items.map((a, idx) => {
        ...
        return `
          <div class="explore-card-wrap" onclick="openModal(${a.id}, '${mediaType}', event)">
            ...
            <div class="explore-card-meta">${typeLabel} ┬╖ Γÿà ${score}</div>
          </div>`;
      }).join('');
      return;
    } catch(e) {
...
function renderRandomPicks(items) {
  ...
  container.innerHTML = items.map(a => {
    ...
    <div class="explore-card" style="width:100%;flex-shrink:1;" onclick="openModal(${a.id}, '${mediaType}', event)">
      ...
      <div class="explore-card-meta" style="font-size:12px;">${typeLabel} ┬╖ Γÿà ${score}</div>
    </div>`;
  }).join('');
}
